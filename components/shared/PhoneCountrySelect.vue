<template lang="pug">
  el-select.select-country-code.select-input-beautiful(ref='dropdownRef' v-model='country_code' :placeholder='placeholder' popper-class='select-beautiful' :filterable='true' :popper-append-to-body='false' @change='onChange' @focus='onFocus')
    template(slot='prefix')
      .flag-current.h-100.d-flex
        span.prefix.flag-icon.m-auto(:class='`flag-icon-${selectedPhoneCountryCode.toLowerCase()}`')
    el-option.text-left(v-for='(country, idx) in filterPhoneCountryList' :key='idx' :label='`${country.dial_code}`' :value='country.dial_code')
      .option
        span.prefix.flag-icon(:class='`flag-icon-${country.code.toLowerCase()}`') 
        span {{ country.dial_code }}
...
import '@/libs/flagIcon/css/flag-icon.min.css';
...
export default class PhoneCountrySelect extends Vue {
  value: string;
  showEmpty: any;
  disabled: any;
  countries = [];
  ...
  fetchData() {
    this.countries = getPhoneCountryList();
  }

  onChange() {
    if (this.country_code || this.showEmpty) {
  ...
  onFocus() {
    ...
    const dropdownRef: any = this.$refs.dropdownRef;
    const dropdownEl: any = dropdownRef.$el;
    const inputInnerEl: any = dropdownEl.querySelector('.el-input__inner');
    inputInnerEl.disabled = true;
    dropdownRef.blur();
  }
  ...
  get filterPhoneCountryList() {
    let list = this.countries;

    list = _.filter(list, (item, idx) => {
      return item.dial_code;
    });

    return _.orderBy(list, (item, idx) => {
      return item.dial_code == '+84' ? 'A' : item.code;
    });
  }

  get selectedPhoneCountry() {
    return _.find(this.countries, (item, idx) => {
      return item.dial_code == this.country_code;
    });
  }

  get selectedPhoneCountryCode() {
    return _.get(this.selectedPhoneCountry, 'code', '');
  }

  get isDisabled() {
    return this.filterPhoneCountryList.length <= 1 || this.disabled;
  }
...
function getPhoneCountryList() {
  let countries = [
    ...
    { name: 'Palau', dial_code: '+977', code: 'PW' },
